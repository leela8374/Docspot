...
const UserAppointments = () => {
  ...
  const handleBooking = async (bookingData) => {
    try {
      const res = await axios.post('http://localhost:8001/api/user/bookappointment', bookingData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      if (res.data.success) {
        ...
        navigate('/userhome'); // Navigate to user home page after successful booking
      } else {
                ...
                doctorAppointments.map((appointment) => {
                  return (
                    <tr key={appointment._id}>
                      ...
                      {
                        appointment.document ? <td><Button variant='link' onClick={() => handleDownload(appointment.document.path, appointment._id)}>{appointment.document.filename}</Button></td> : 'No Doc'
                      }
